...
function processContact() {
  ...
  setupFiles();
  processXLSFilesInZip(contactFolder_);
...
function processXLSData(folder, xlsBlob) {
  ...
  if (folder.getName() == "Survey Response Reports") {
    ...
    surveyData
      .getRange(2, surveyData.getLastColumn() - 2, importedData.length, 1)
      .setFormula("=COUNTIF(A$1:A2,A2)");
    surveyData
      .getRange(
        startRow,
        surveyData.getLastColumn() - 1,
        importedData.length,
        1
      )
      .setValues(filename);
    surveyData
      .getRange(startRow, surveyData.getLastColumn(), importedData.length, 1)
    ...
    contactData
      .getRange(2, contactData.getLastColumn() - 2, importedData.length, 1)
      .setFormula("=COUNTIF(A$1:A2,A2)");
    contactData
      .getRange(
        startRow,
        contactData.getLastColumn() - 1,
        importedData.length,
        1
      )
      .setValues(filename);
    contactData
      .getRange(startRow, contactData.getLastColumn(), importedData.length, 1)
      .setValues(dateTimeCol);
  }
}
